...
func (s *Scene) AddLight(l Light) (err os.Error) {
	if l == nil {
...
func (s *Scene) AddMaterial(name string, m Material) (err os.Error) {
	return os.NewError("We don't support named materials yet")
}

// AddObject adds a three-dimensional object to the scene.
func (s *Scene) AddObject(obj Object3D) (id ObjectID, err os.Error) {
	id = s.nextFreeID
